...
function removeFirstOccurrences(str, value) {
  const indexFirstChar = str.indexOf(value);
  if (indexFirstChar === -1) {
    return str;
  }
  const indexEndChar = indexFirstChar + value.length;
  return str.slice(0, indexFirstChar) + str.slice(indexEndChar);
}
...
function removeLastOccurrences(str, value) {
  const indexFirstChar = str.lastIndexOf(value);
  if (indexFirstChar === -1) {
    return str;
  }
  const indexEndChar = indexFirstChar + value.length;
...
function formatTime(minutes, seconds) {
  const setMin = String(minutes).padStart(2, '0');
  const setSec = String(seconds).padStart(2, '0');
  return `${setMin.padStart(2, '0')}:${setSec.padStart(2, '0')}`;
}
...
function findLongestWord(sentence) {
  ...
  sentenceSplit.forEach(function getLong(word) {
...
function reverseWords(str) {
  const arr = str.split(' ');
  const reverseArr = arr.map(function getReverseWord(word) {
